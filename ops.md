...
| `upsample_bilinear2d.vec` (14), explicit output sizes and `align_corners=true` | CSATv2 | Add a real bilinear-resize op: shape inference, coordinate transform, boundary handling, direct/symbolic kernel, and targeted oracle tests. **Landed 2026-08-26; scope widened before landing to a real `align_corners` parameter and a `scale_factors` entry point (see `todo.md`'s "remove unnecessary specialization" audit, requested alongside this row).** `Resize.Bilinear2d` (`lib/native/ops/resize.ml`): `Bilinear_axis.endpoints` needs no new `SEMANTICS` primitive — `in_extent`/`out_extent` are static graph-construction-time extents, not tensor data, so both ATen coordinate-transform formulas (`align_corners` true and false) reduce to an EXACT integer floor division and remainder over the existing `index_scale`/`index_floor_div_pos` idiom (`Window_axis`/`Pool.Adaptive_axis`'s own pattern), not a new `Expr.Intrinsic` node the way `Max_pool2d`'s window geometry needed. One aggregate bound (`Shape_error.Resize`) covers both formulas' numerators, checked in `output_shape` before `Compute` multiplies the factors. `scale_factors` is resolved to an explicit `output_size` at import time in both `Op_bridge` and `Native_interp` (ATen's own `floor(input_size * scale_factor)`, against the real input shape each importer already has) — the Native op only ever sees a size, so this is the sanctioned "translate params, still one node" pattern, not a decomposition; both importers reject a graph giving neither or both of `output_size`/`scale_factors`. **No real ATen C binding**: `UpSampleBilinear2d.cpp`/`cpu/UpSampleKernel.cpp` compile cleanly against the curated archive in isolation, but `scale_factors`'s `float[]?` argument exposed a real gap in the binding generator's optional-list convention (`Optional (List (Base ..., _))` collapses "absent" and "present-but-empty" to the same `[]`, which would silently turn an absent `scale_factors` into a present-empty `ArrayRef` and fail ATen's own contract check on every accepted graph) — fixing that generally is out of scope here, so `bin/aten_ops_gen.ml` does not select this op, the same treatment `group_norm.default` got. Correctness is instead pinned by hand-derived `compute_test.ml` fixtures (both `align_corners` values, plus the `out_extent=1` degenerate case), the mandatory Direct-vs-Symbolic bitwise agreement (native and Native4D), a new `lib/native_op_walk/upsample_bilinear2d_nwalk.ml` fuzz walk, and hand-derived `dispatch:` fixtures in both `test/native_bridge_test.ml` and the new `test/native_interp/upsample_test.ml` — including one fixture per importer proving the `output_size` and `scale_factors` entry points build the identical graph. The 100-model sweep was not regenerated for this landing (gated on downloaded release archives, not run this session). | A natural `ResizeBilinear4` can operate on H/W/C, but must reject/diagnose unsupported axes rather than reinterpret them. **Landed 2026-08-26.** No new Native4D payload: `output_size`/`align_corners` name no axis and carry no shape, so `Ops4`/`Domain`/`Output_transfer4` reuse `Resize.Bilinear2d.t` unchanged (the same treatment `Adaptive_avg_pool2d` gets) and `lower.ml`'s arm is a direct, unconditional passthrough like `Max_pool2d`'s. |
