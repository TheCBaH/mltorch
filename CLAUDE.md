...
- **Never assume a 63-bit `int` in the JS-reachable libraries** — `lib/native`,
  ...
  JavaScript uses no bitwise operator on an index), `lib/js_ast` (its printer never
  formats an OCaml `int` into the output: literals go through
  `float_of_int`/`Int64.to_string`, and Melange mirrors it). js_of_ocaml's `int` is **32 bits** (`Sys.int_size = 32`), so a value
...
make jsoo.inline-runtest     # the expect suites under node (@runtest-js), incl. the Loop IR JS run in-process
...
out. Melange stops at `walk_core` + `core` + `js_ast` (no Bigarray in any release), reaches no
