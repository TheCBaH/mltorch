...
module Intrinsic : sig
  ...
  module Max_pool : sig
    ...
    (** The spelling used in printed output, alongside [Value.unary_name] and
        [Value.binary_sym]. Named rather than inlined at the printer because it
        is part of a rendered form that expect tests pin. *)
  ...
  (** The half-open input window an output position reads, clipped to the input
      ...
      [Ground_eval] expands the same stencil, and as a consumer it cannot reach
      the library-private checked arithmetic. If only [Eval] went through this,
      the verifier's copy would stay unchecked and the two would drift — the
      same failure mode [Max_op] exists to prevent. Validating the descriptor's
...
module Fold : sig
  ...
  (** Node count, index trees included — a load's addressing is where the bulk
      of a large expression lives, so treating it as a leaf would measure almost
      nothing. Unmetered: it walks the whole tree, which is why [Check.value]
      does not use it to enforce a limit. *)

  val depth : Value.t -> int
  (** Nesting depth, index trees included, for the same reason as [size]. Also
      unmetered, and computed by the same traversal, so the two cannot disagree
  ...
  (** How many [Index.assume_position] claims the expression contains.
      [assume_position] proves nothing — it records an author's claim — so being
      able to locate every one is the point of keeping it a distinct node. *)
  ...
  (** Binders in lexical order, with repeats — one identity bound in two sibling
      scopes appears twice, which is what separates counting binders from
      counting identities. Inspection only: [Pp] and the structural comparison
      each carry a scoped environment instead, since a list keyed by identity
      cannot tell those siblings apart. *)
end
...
module Rewrite : sig
  ...
  (** Replaces every BOUND reducer identity consistently; free ones are left
      ...
      outer one. Freshening the combined tree afterwards repairs nothing — once
      a nominal collision has captured a reference, there is no record of which
      binder it meant. *)

  val substitute_output : Role.Position.t Index.t Coord.t -> Value.t -> Value.t
  ...
  (** Deterministic renaming by lexical traversal — binders take the lowest
      ordinals NOT occurring free in the expression, in the order they are met:
      0, 1, ... for a closed expression, skipping the free ones otherwise, since
      [freshen] must not capture them. Canonical either way, because
      alpha-equivalent expressions have the same free set and so skip the same
      ordinals. Does not reorder operations or reductions. Idempotent. *)
...
module Check : sig
  ...
  (** Deliberately narrow. Division parameters, load roles and intrinsic
      ...
      never be turned red, and CLAUDE.md is explicit that a check which has
      never failed is not evidence. What remains is what COMPOSITION can still
      violate — a free reducer, a binder shadowing itself (how two unfreshened
      fragments capture each other), and the optional size/depth limits.

      A configured limit is checked FIRST and is metered, stopping at the first
      node past it: the scope traversals recurse over the whole tree, so running
      them first would exhaust the stack on exactly the oversized input the
      limit exists to reject. An expression that is both oversized and
      ill-scoped is therefore reported as oversized.
...
module Eval : sig
  ...
  (** Everything index evaluation can raise, and no more. [error] widens this in
      stage 4 with the load and intrinsic cases. The first two come from the
      library-private checked arithmetic; a public row may name a private
  ...
  (** The reference interpreter.
      ...
      engine — a rewrite that reassociated it would change the answer, not just
      the shape. Max-pool advances value and index together under
      [Max_op.pool_better]; updating them separately is how they originally fell
      out of step.

      Internally it recurses through a private exception and converts once here,
      rather than allocating an [Ok] per node per output pixel: this runs inside
      the grounding loop the transform verifier drives for every walk config. *)
end

module Pp : sig
  val index :
    names:(Reduce_var.t -> string) -> Format.formatter -> 'role Index.t -> unit
  ...
  (** Assigns reducer display names r1, r2, ... in LEXICAL order, so two
      ...
      up a sibling's binder. Every form renders exactly as the representation it
      replaced — [Round_f32] is the one genuinely new node and has no old
