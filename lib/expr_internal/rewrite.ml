...
let rec rebuild ~idx ~src ~on_load ~on_i64_load ~on_i64_local ~on_i64_local_at
  ...
  | Value.Scan_at (s, row, lane) ->
      ...
      let init, st =
        rebuild ~idx ~src ~on_load ~on_i64_load ~on_i64_local ~on_i64_local_at
          ~on_local ~on_local_at ~on_local_scan_at ~on_reduce ~on_local_bind
          env1 lenv s.Scan.init st
      in
...
(* [Float_to_i64]'s operand is an ordinary [float Value.t] child, rebuilt
   ...
   [on_i64_load], the [int64 Value.t] twin of [on_load]; [I64_local]/
   [I64_local_at] go through [on_i64_local]/[on_i64_local_at], the [int64
   Value.t] twins of [on_local]/[on_local_at]. *)
and rebuild_i64 ~idx ~src ~on_load ~on_i64_load ~on_i64_local ~on_i64_local_at
    ~on_local ~on_local_at ~on_local_scan_at ~on_reduce ~on_local_bind env lenv
    (e : int64 Value.t) st =
  let go_i64 =
    rebuild_i64 ~idx ~src ~on_load ~on_i64_load ~on_i64_local ~on_i64_local_at
      ~on_local ~on_local_at ~on_local_scan_at ~on_reduce ~on_local_bind env
      lenv
  in
  let idxe i = idx.on_index env i in
...
and rebuild_bool ~idx ~src ~on_load ~on_i64_load ~on_i64_local ~on_i64_local_at
  ...
  | Bool.I64_eq (x, y) ->
      let x, st =
        rebuild_i64 ~idx ~src ~on_load ~on_i64_load ~on_i64_local
          ~on_i64_local_at ~on_local ~on_local_at ~on_local_scan_at ~on_reduce
          ~on_local_bind env lenv x st
      in
      let y, st =
        rebuild_i64 ~idx ~src ~on_load ~on_i64_load ~on_i64_local
          ~on_i64_local_at ~on_local ~on_local_at ~on_local_scan_at ~on_reduce
          ~on_local_bind env lenv y st
      in
      (Bool.I64_eq (x, y), st)
  | Bool.I64_lt (x, y) ->
      let x, st =
...
let freshen e s =
  ...
  rebuild
    ...
    ~on_i64_local:(fun _lenv v st -> (Value.I64_local v, st))
    ~on_i64_local_at:(fun _lenv v i st -> (Value.I64_local_at (v, i), st))
...
let substitute_locals f e st =
  rebuild ~idx:keep_indices ~src:Fun.id ~on_load:keep_load
    ~on_i64_load:keep_i64_load
      (* [local_binding] (below) is float-only -- a [Scalar]/[Vector] payload is
       always a [float Value.t] -- so an I64 local can never be found in [f]'s
       table yet; both pass through unchanged, matching [on_local]'s own
       [None] arm. Typed substitution here is P3.3/P3.4's job, once Region
       gains typed local storage. *)
    ~on_i64_local:(fun _lenv v st -> (Value.I64_local v, st))
    ~on_i64_local_at:(fun _lenv v i st -> (Value.I64_local_at (v, i), st))
