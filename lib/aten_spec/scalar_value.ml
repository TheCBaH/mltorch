(* A scalar literal: an op's Scalar argument, or one explicit tensor value.
   ...
   and floats stay bit-exact: {"int": 1} | {"float": "0x3f800000"} | {"bool":
   true}. As a convenience a bare JSON bool decodes to [Bool] and a bare number
   to [Float] (rounded to f32) — but a bare number can't be an [Int] (JSON has
   one number type), so use the object form when an integer scalar is required. *)
...
let jsont : t Jsont.t =
  Jsont.map ~kind:"scalar"
    ...
    ~enc:(function
      ...
      | Float f ->
          Spec_util.single ~case:"float" (Spec_util.jstr (Float32.to_hex f))
