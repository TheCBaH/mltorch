...
let emit_param (a : Aten_func_ast.Argument.t) ~kwonly =
  ...
  | Some ty_s ->
      ...
      in
      Some
        (Printf.sprintf "{ name = %S; ty = %s; default = %s; kwonly = %b }"
           a.name ty_s default_s kwonly)
...
let config_record (op : A.t) =
  if op.arguments.out <> [] then None
    ...
    match returns_arity op.returns with
    | None -> None
    | Some ret_str ->
        let args =
...
let file (ops : A.t list) =
  ...
  let all_entries =
    List.filter_map
      (fun op ->
        match config_record op with
        | None -> None
        | Some _ ->
            Some (Printf.sprintf "  (%S, %s)" (target op) (ocaml_name op)))
      ops
  in
  String.concat "\n\n"
    [
