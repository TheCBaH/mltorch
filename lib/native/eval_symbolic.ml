...
(* [Eval_direct]'s mixed-dtype fix (a mismatched I64/F32 pair fails at checked
   ...
   [Compute_i64] wiring for these ops), so a matching I64,I64 pair is unaffected
   and unchanged. *)
let check_mixed_dtype (gr : graph) op =
  ...
  let check_pair mixed_op a b =
    ...
    (* Arithmetic on Bool stays rejected, checked FIRST so a Bool paired with
       I64 reports the Bool reason, not
       the unrelated I64-mixing one below -- matching [Eval_direct]'s own
       precedence for the identical check on the Direct route. *)
...
(* An exact int64 pixel for an I64-formatted [Factory.Arange] node:
   ...
   count it admits, so a plain modular
...
let process_node ~limits ~fill ~pixel (gr : graph) (env, stages, stages_i64)
  ...
  (* The Symbolic twin of [Eval_direct]'s own dtype-preserving [Reshape]
       ...
       pixel is a deferred expression, not an immediate value. An int64-to-int64
       consumer needs nothing more: [Kernel.Value_i64.t]'s existing
       forward-reference
