...
module type SEMANTICS = sig
  ...
  (* IEEE numerical equality ([Expr.Bool.Value_eq]'s semantics target): NaN
     ...
     [Pointwise.To_copy]/[Bitwise_not]'s Bool-cast formula (this primitive is
     the fix). *)
  val eq : t -> t -> b
  val select : b -> t -> t -> t
...
(* Carrier-indexed sibling of [SEMANTICS], added alongside it rather than
   ...
   - [input] stays a single, non-carrier-indexed type (matching [SEMANTICS]'s
     ...
     grounding-time check) -- no real caller yet needs one checked entry point
     across formats, so that stays open against Region's own typed output-group
     work.
   - Arithmetic/comparison/cast are named per concrete carrier
     ...
     non-GADT type outside [_ value] (both deliberate choices) -- unifying the
     op
