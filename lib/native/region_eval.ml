...
(* Exact int64 counterpart of [materialize]'s pixel-degenerate case: no
   ...
   [Region_local_i64]/[Region_slots_i64] (P3.3) already cover the
   locals-filling piece in isolation. Building the combined
   partition-carrying wrapper ahead of a real caller (P5.1's Arange, which
   needs exactly this no-locals shape, or P5.5's mixed value/index groups,
   which need the harder combined one) would repeat the premature
   abstraction this plan's own P2.2/P3.1/P3.2 slices already declined --
   see the implementation tracker's P3.4 note. *)
