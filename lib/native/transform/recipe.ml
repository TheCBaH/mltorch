...
type 'v insertion = { op : op; outputs : 'v target list; from : Node_id.t list }
...
let existing id s =
  match Snapshot.edge s.snapshot id with
  ...
  emit
    {
      ...
      value_claims =
        List.map
          (fun (from, onto) -> (from, Preserved onto, Correspondence.Identical))
          tie;
    }
...
let pp_replacement fmt r =
  ...
  let pp_subst fmt (from, onto) =
    Fmt.pf fmt "@[<h>%a := %a@]" Tensor_id.pp (raw_edit_edge from) Tensor_id.pp
      (raw_edit_edge onto)
  in
