...
module Make (D : Dialect.S) = struct
  ...
  let pp_mismatch fmt = function
    ...
    | Escapes id ->
        Fmt.pf fmt "@[<h>%a is used outside the match or is a graph output@]"
          Tensor_id.pp id
    | No_progress id ->
        Fmt.pf fmt "@[<h>chain step did not move upstream of %a@]" Tensor_id.pp
          id
    | Not_constant id ->
        Fmt.pf fmt "@[<h>%a is not a constant@]" Tensor_id.pp id
    | Rejected -> Fmt.string fmt "rejected by a guard"
    | Shared id ->
        Fmt.pf fmt "@[<h>%a has more than one consumer@]" Tensor_id.pp id
    | Unproduced id ->
        Fmt.pf fmt "@[<h>%a is a graph input, not a node output@]" Tensor_id.pp
          id
    | Wrong_op id -> Fmt.pf fmt "@[<h>%a is not the expected op@]" Node_id.pp id

  type failure = Invalid of Rgn.error | Mismatch of mismatch

  let pp_failure fmt = function
    | Invalid e -> Rgn.pp_error fmt e
  ...
  let producer ~take edge s =
    match View.def s.view edge with
  ...
  let is_graph_output edge =
    let* v = view in
    return (View.is_graph_output v edge)

  let interior edge =
    let* v = view in
    if View.is_graph_output v edge then fail (Escapes edge)
    else
      match View.uses v edge with [ _ ] -> return () | _ -> fail (Shared edge)

  let constant edge =
    let* v = view in
    if View.is_constant v edge then return () else fail (Not_constant edge)

  let sig_of edge =
    let* v = view in
    match View.sig_of v edge with
    | Some sg -> return sg
  ...
  let scan pattern v =
    let anchors =
      List.concat_map
        ...
        (Graph_common.nodes (View.graph v))
    in
        ...
        (fun (acc_exclusive, acc_touched, found) anchor ->
          ...
          | Ok (value, s) -> (
              ...
              then (acc_exclusive, acc_touched, found)
                ...
                match Rgn.of_nodes s.view touched with
                | Error _ -> (acc_exclusive, acc_touched, found)
                | Ok r ->
