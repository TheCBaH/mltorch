...
type t = {
  ...
  outputs : Tensor_id.t list; (* claimed edges used outside, or graph outputs *)
...
(* [convex] is REPORTED, not enforced. Non-convexity — a path that leaves the
   ...
   general backstop. *)
val of_nodes : Graph_view.t -> Node_id.Set.t -> (t, error) Core.result
...
val extract : Graph_view.t -> t -> graph
