...
module Make (S : Side.S) = struct
  ...
  type error =
    [ Rw.error | `Not_converged of string | `Verification of Verification.t ]
  ...
  type 'v outcome = { audits : Audit.t list; step : 'v Rw.step }
  ...
  let verified name ctx state (Rw.Step (_, map) as step) =
  ...
  let fixpoint ?(max_iters = 16) inner =
    {
      ...
      run =
        (fun ctx state ->
          let rec go : type v.
              int -> Audit.t list -> v Rw.step -> (v outcome, error) Core.result
              =
           fun fuel audits (Rw.Step (state, acc)) ->
            if fuel <= 0 then Core.fail (`Not_converged inner.name)
              ...
              if changed map then
                go (fuel - 1) audits (Rw.Step (next, Graph_map.compose acc map))
              else Core.return { audits; step = Rw.Step (state, acc) }
          in
  ...
  let run_with ctx state passes =
    let rec go : type v. v Rw.t -> t list -> (v outcome, error) Core.result =
     fun state passes ->
      match passes with
