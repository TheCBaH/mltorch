...
let rec eval e v =
  match e with
  | Binary (op, a, b) -> Expr.Value.apply_binary op (eval a v) (eval b v)
  ...
  | Unary (op, x) -> Expr.Value.apply_unary op (eval x v)

and eval_guard g v =
  match g with
  | Lt (a, b) ->
      (* Inlined: [Ground_expr] has its own [Lt] guard and needs no separate
         compare_op type to carry the same operator twice. *)
      eval a v < eval b v
  | Pool_better { best; value } ->
...
let rec pp fmt = function
  | Binary (op, a, b) ->
      Fmt.pf fmt "(%a %s %a)" pp a (Expr.Value.binary_sym op) pp b
  ...
  | Unary (op, x) -> Fmt.pf fmt "%s(%a)" (Expr.Value.unary_name op) pp x
...
let normalise ~stored_f32 e =
  ...
  let rec go blocked = function
    | Binary (op, a, b) ->
        ...
        ( blocked,
          fold2 (fun a b -> Binary (op, a, b)) (Expr.Value.apply_binary op) a b
        )
    ...
    | Unary (op, x) -> (
        ...
        ( blocked,
          match closed x with
          | Some v -> Const (Expr.Value.apply_unary op v)
          | None -> Unary (op, x) ))
