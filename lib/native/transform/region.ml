...
let pp_error ppf : [< error ] -> unit = function
  ...
  | `Unknown_node id -> Fmt.pf ppf "region claims unknown node %a" Node_id.pp id
...
let ordered view nodes =
  ...
  |> List.filter_map (fun id -> Graph_view.node view id)
  |> List.sort (fun (a : node) (b : node) ->
      Int.compare
        (Option.value (Graph_view.topo_index view a.Node.id) ~default:0)
        (Option.value (Graph_view.topo_index view b.Node.id) ~default:0))
  ...
  let rec walk visited = function
    ...
    | (n : node) :: rest ->
        if claimed n.Node.id then false
          ...
          let next =
            List.concat_map (fun id -> Graph_view.uses view id) n.Node.outputs
          in
...
let of_nodes view nodes =
  ...
  let* () =
    Node_id.Set.fold
      (fun id acc ->
        let* () = acc in
        match Graph_view.node view id with
        | Some _ -> Core.return ()
  ...
  in
  let inputs =
    List.concat_map (fun (n : node) -> Graph_ir.operands n.Node.op) members
  ...
  Core.return
    { nodes; inputs; outputs; interior; convex = is_convex view nodes outputs }
...
let extract view t =
  ...
  let input_kinds =
    List.fold_left
      (fun acc id ->
        if Graph_view.is_constant view id then
...
let pp fmt t =
  ...
  Fmt.pf fmt
    ...
    (ids Tensor_id.pp) t.inputs (ids Tensor_id.pp) t.outputs (ids Tensor_id.pp)
    t.interior t.convex
