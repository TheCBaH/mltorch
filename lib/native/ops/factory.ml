...
module Arange = struct
  (* The exact int64 view of [params]' bounds, ALONGSIDE the float fields
     every existing caller already uses -- never replacing them. [Some] only
     when every
     ...
     and used by [length_exact] below even though [value_i64_exact] itself only
     needs [start]/[step]. *)
  module Exact = struct
  ...
  (* [length]'s exact int64 twin: the element count computed entirely in
     [int64] ceiling division, with no float round trip -- fixes a count gap
     (real ATen and a
  ...
  (* [checked_mul]/[checked_add] exist because [Int64.mul]/[Int64.add] wrap
     silently on overflow -- exactly the failure mode of computing start + i *
     step with unchecked overflow. Not a
  ...
  (* [value]'s exact int64 twin: [start + i*step] computed entirely in
     ...
     [Some], fixing an exact-truncation bug. [i] is always well under 2^31 by
     [length]'s own
