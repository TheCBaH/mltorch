...
struct
  ...
  let window ~(kernel : Dim.extent Dim.t) ~(stride : Op_config.Pos.t)
      ~(pad : Op_config.Nonneg.t) ~(in_extent : Dim.extent Dim.t)
      (out_axis : Semantics.position S.index) : window =
    ...
    let base = S.index_add (S.index_const pad) (S.index_scale (-stride) out) in
    let lo = S.clamp_low base in
    let hi =
      S.index_min (S.index_extent kernel)
        (S.index_add (S.index_extent in_extent) base)
    in
    (* [assume_index]: for k in [lo,hi), stride*out + k - pad is in [0,in_extent) *)
    let src k =
      S.assume_index
        (S.index_add (S.index_scale stride out)
           (S.index_add (S.of_index k) (S.index_const (-pad))))
    in
    { lo; hi; src }
end
