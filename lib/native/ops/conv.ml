(* Convolution ops. This file is now a thin facade: Conv1d (which depends on
   ...
   Conv2d_padding (which depends on Conv2d) in conv_padding.ml, and
   Convolution (which also depends on Conv2d) in conv_convolution.ml. Every
   alias below is exactly what was a top-level module in this file before the
   split, so external references such as [Conv.Conv2d] or [Conv.Convolution]
   are unaffected.. *)
