(* Assembles Div_scalar's walk: its config space (Pointwise.Scalar_bin.Walk, applied to the
  ...
  let build pcg c =
    ...
    let g =
      Core.or_raise Graph_builder.pp_error
        Graph_builder.(
          build ~name:"div_scalar" ~outputs:(fun r -> [ r ])
          @@
          let* xi = input ~shape ~name:"x" () in
          div_scalar ~name:"out" c.W.scalar xi)
    in
