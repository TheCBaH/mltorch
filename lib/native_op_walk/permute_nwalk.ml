(* Assembles Permute's walk: config from Permute.Permute.Walk (a full 6-axis
   shape plus a permutation drawn from a fixed candidate set, weighted away
   from identity), one-input graph. The first Permute walk -- op3-impl.md row
   3.3b, F5. *)

module M = struct
  module W = Permute.Permute.Walk (Walk_limits.L)
  include W

  type subject = Native_subject.t

  let target = "permute"

  let build pcg c =
    let shape = W.shape c in
    let x, pcg = Native_tensor.synth pcg shape in
    let g =
      Err.or_raise ~pp_error:Graph_builder.pp_error
        Graph_builder.(
          build ~name:"permute" ~outputs:(fun r -> [ r ])
          @@
          let* xi = input ~shape ~name:"x" () in
          permute ~name:"out" (W.perm c) xi)
    in
    let inputs = List.combine g.Graph_ir.Graph.inputs [ x ] in
    ({ Native_subject.target; graph = g; inputs }, pcg)
end
