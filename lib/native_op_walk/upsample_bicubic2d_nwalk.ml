(* Assembles Upsample_bicubic2d's walk: the same ad hoc (n, c, input_h,
   input_w, out_h, out_w, align_corners) config space as
   [Upsample_bilinear2d_nwalk], mirroring it exactly -- no pair needs
   correlating: [Resize.Bicubic_axis.check] only rejects when the
   coordinate-transform aggregate overflows a hard limit far above anything
   this walk's small literal bounds can reach, so every combination in range
   is valid by construction (same F4 instance the adaptive-pool walk's header
   cites). *)
...
module M = struct
  ...
  let params c : Resize.Bicubic2d.params =
    {
      output_size =
        {
  ...
  let build pcg c =
    ...
    let g =
      Err.or_raise ~pp_error:Graph_builder.pp_error
        Graph_builder.(
          build ~name:"upsample_bicubic2d" ~outputs:(fun r -> [ r ])
          @@
          let* xi = input ~shape ~name:"x" () in
          upsample_bicubic2d ~name:"out" (params c) xi)
    in
