(* [Gt_scalar]'s first walk (plan S6/T6.4/W); see [Eq_scalar_nwalk]'s own
  ...
  let build pcg c =
    ...
    let g =
      Err.or_raise ~pp_error:Graph_builder.pp_error
        Graph_builder.(
          build ~name:"gt_scalar" ~outputs:(fun r -> [ r ])
          @@
          let* xi = input ~shape ~name:"x" () in
          gt_scalar ~name:"out" c.W.scalar xi)
    in
