(* Assembles Hardsigmoid's walk: its config space (Pointwise.Hardsigmoid.Walk,
   applied to the shared limits) plus the native-graph [build]. *)

module M = struct
  module W = Pointwise.Hardsigmoid.Walk (Walk_limits.L)
  include W

  type subject = Native_subject.t

  let target = "hardsigmoid"
