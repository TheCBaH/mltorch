...
[Value.i64_of_index], the exact index-to-int64 counterpart of
[Value.value_of_index], is pinned to [int64 t] like every other [int64 t]
constructor: its result must not typecheck where a [float t] is expected.

  $ check "i64_of_index rejects use as a float operand" "ignore (Expr.Value.add (Expr.Value.const 1.) (Expr.Value.i64_of_index (Expr.Index.const 0)))"
  i64_of_index rejects use as a float operand: rejected
  $ check "i64_of_index combines with other int64 t values" "ignore (Expr.Value.i64_add (Expr.Value.i64_const 1L) (Expr.Value.i64_of_index (Expr.Index.const 0)))"
  i64_of_index combines with other int64 t values: COMPILES
