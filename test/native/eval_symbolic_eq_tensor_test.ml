(* [Eq_tensor]'s Symbolic/Kernel route (P6.4/P6.5): [Eval_symbolic.run] builds an
   ordinary float [Stage.t] from the [SEMANTICS]-generic 0./1. formula, and
   [Graph_builder.eq_tensor] declares the output edge [Bool]. Bool storage is
...
let%expect_test
    "Symbolic -> Kernel: Eq_tensor's Bool-declared output is stored as a Bool \
     payload" =
  let kernel =
