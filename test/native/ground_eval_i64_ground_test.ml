(* [Ground_eval.at] has no grounded/fused representation for [I64_to_float]
   yet (Gate 1's typed Expr foundation only wires the plain evaluator,
   [Eval.value]); it must reject explicitly rather than silently miscompiling
...
let%expect_test "Ground_eval.at rejects I64_to_float outright" =
  ...
  [%expect
    {| I64_to_float has no grounded/fused representation yet (P1 typed Expr foundation only supports the plain evaluator, see .ai/) |}]
