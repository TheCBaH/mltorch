...
(* THE CLAIM, historically: [To_copy]'s [Float] pixel used to be [S.load x
   ...
   `eval_symbolic.ml`'s own explicit-cast arm (implementation tracker's P5.3
   continuation, "To_copy(Float)'s own Symbolic dispatch"), [To_copy(Float)]
   ...
   untouched; `native_interp` never calls this Symbolic evaluator at all --
   see the tracker's P5.7 note), but [Ground_eval] -- the separate evaluator
   ...
   named, already-tracked P1 typed-Expr-foundation gap, not a fresh defect:
   see [Ground_eval]'s own `Unsupported_i64_to_float_ground`). The cluster
   ...
   Extending [Ground_eval] to evaluate [I64_to_float]/[I64_load] is tracked,
   not attempted here -- see the implementation tracker's own P5.3 entry. *)
  ...
  | Ok (Rewrite.Origin state) ->
      ...
      [%expect
        ...
        pass fold_arange_cast rejected: 3 clusters: 1 proved (structural), 1 unproved (grounding failed), 1 vacuous
          ...
          {t1} -> {t1} identical: unproved: eval: I64_to_float has no grounded/fused representation yet (P1 typed Expr foundation only supports the plain evaluator, see .ai/) [exhaustive]
