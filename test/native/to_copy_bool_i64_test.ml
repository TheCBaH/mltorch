(* [To_copy]'s [Bool] target on an I64 operand: design section 3's "I64 to
   Bool: Exact comparison with 0L" -- previously rejected outright
   (`Unsupported_to_copy_bool_source`), since [To_copy(Bool)]'s own P6.3
   landing (commit `b419248f`) only wired the F32 source EdgeNeXt's own mask
