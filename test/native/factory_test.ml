...
(* Above 2^53, [start]/[step] as plain [float]s can no longer represent every
   ...
   entirely and generate the real int64 values directly. *)
...
(* [start] near [Int64.max_int] with a [stop] built by an unchecked
   ...
   once [length] routes through [length_exact], this is
...
(* [value_i64_exact] called directly, bypassing [length_exact]: proves the
   generation-time [checked_add] overflow guard is still real, for any caller
   that reaches an individual index without first
   deriving it from a self-consistent [length_exact] count (e.g. a
   hand-built [Kernel.Value_i64.t]) -- [length_exact]'s own success on a
   *consistent* range makes every in-range index provably non-overflowing, so
   this guard is now defense in depth,
