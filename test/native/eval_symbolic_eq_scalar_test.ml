(* [Eq_scalar]'s Symbolic/Kernel route (P6.4): unlike the Direct-route
   dispatch arm ([Eval_direct]'s own [Eq_scalar] case, exercised end to end
   by [eq_scalar_bool_test.ml]), Symbolic has no
   [Compute_i64]-style Bool-aware arm and [Eval_symbolic.check_mixed_dtype]
   does not list [Eq_scalar] at all, so [Eval_symbolic.run] builds an
   ordinary float [Stage.t] for it -- the same [SEMANTICS]-generic 0./1.
   formula [pointwise_test.ml]'s own "Direct: eq_scalar" fixture exercises
   directly. [Graph_builder.eq_scalar] still declares the output edge [Bool]
   ...
   [Kernel.materializable] F32-only gate [Gt_scalar]'s own analogous fixture
...
let build =
  Graph_builder.(
    build ~name:"eq_scalar_symbolic" ~outputs:(fun r -> [ r ])
    @@
...
let%expect_test
    "Symbolic -> Kernel: Eq_scalar's Bool-declared output is rejected at \
