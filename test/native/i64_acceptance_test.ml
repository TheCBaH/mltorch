(* End-to-end acceptance fixtures for real multi-node I64 forcing shapes named
   by the corpus: each earlier fixture tested one [Compute_i64] arm at a time,
   fed by a raw [input] tensor
   ...
   from the real exports) through [Graph_builder]
...
(* mvitv2_tiny's relative-position-bias pattern: every one of its 40
   ...
   `other=1.0` (-> [Mul_scalar]). [Reshape] here is fed by [Arange]'s own OWN
   computed edge, not a
...
let%expect_test
  ...
  (* self + 52.0 = {-1.5, 51.5, 52.3, 54.7}; truncated toward zero, per the
     explicit truncation-toward-zero policy, not rounded. *)
  [%expect {| out = tensor i64 [C=4] {-1, 51, 52, 54} |}]

(* EdgeNeXt's own named pattern: an I64 `arange.default`
