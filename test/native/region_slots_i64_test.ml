(* Exercises [Region_local_i64]/[Region_slots_i64] (P3.3's own remainder note
   in the implementation tracker: the typed scratch-segment machinery, kept
   deliberately SEPARATE from [Region_local]/[Region_slots] rather than
   generalizing them -- see the tracker's D08 note -- so every existing
   float-only Region caller stays untouched). No real op wires this into
   [Region_program.t] yet (that combined-segment question is P3.4/P5's own),
