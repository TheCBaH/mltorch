(* [Eval_direct]'s explicit int64-input promotion for [Pointwise.To_copy]'s
   [Float] target (a follow-up to [Mul_scalar]'s own explicit-cast fix):
   ...
   [Tensor.read_i64_at6] and promotes with an explicit [i64_to_float]
   (integer-to-float is always an explicit expression cast). This is
   the EdgeNeXt/mvitv2 "I64 Arange -> Float cast" acceptance pattern's own
   promoted-consumer step, the same architecture-only shape
