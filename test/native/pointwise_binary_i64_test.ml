(* [Eval_direct]'s dtype-preserving tensor-tensor Add/Sub/Mul (P5.3 follow-up
   to Reshape/Permute/Mul_scalar's own exact-I64 dispatch): the default
   ...
   the fmt-threading regression class recorded in the implementation tracker's
   P5.2 note -- unlike [Mul_scalar] (output stays float by design), a genuine
...
(* Arithmetic on Bool stays rejected (design contract, Gate 6 item 3):
   before this check, a genuine [Payload.Bool] operand paired with an F32 one
   ...
   -- no validated promotion policy exists for that either, the same P5.4
   reasoning the I64 case above already applies, just for a different
