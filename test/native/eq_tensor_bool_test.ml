(* [Eval_direct]'s [Eq_tensor] dispatch arm (P6.4), end to end through
   [Graph_builder.eq_tensor]: [pointwise_test.ml]'s own "Direct: eq_tensor"
   exercises [Compute]'s [SEMANTICS]-generic formula directly, and
   [eval_symbolic_eq_tensor_test.ml] proves the Symbolic/Kernel route
...
let%expect_test "Direct graph: eq_tensor writes genuine Bool storage" =
  let result =
    let open Err.Syntax in
    let* g =
      lift_build
        Graph_builder.(
          build ~name:"eq_tensor_direct" ~outputs:(fun r -> [ r ])
          @@
  ...
  (* Matches [pointwise_test.ml]'s own "Direct: eq_tensor" values exactly. *)
  [%expect {| tensor bool [C=5] {1, 0, 1, 0, 1} |}]
