(* [Region_eval.materialize_i64] (the no-locals slice: the exact int64
   pixel-shaped materialize -- one evaluation of the output expression
   ...
   independently-owned tensor per call ("a subsequent invocation cannot mutate a
   returned tensor"). *)
