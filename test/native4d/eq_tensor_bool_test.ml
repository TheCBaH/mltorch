(* [Eval_direct4]'s [Eq_tensor] dispatch, the Native4D twin of
   [Eval_direct]'s own P6.4 arm ([eq_tensor_bool_test.ml] under test/native).
...
let%expect_test "direct4: eq_tensor writes genuine Bool storage" =
  let g =
    Builder.build
      ~outputs:(fun o -> [ o ])
      (let open Builder in
  ...
  (* Matches Native's own "Direct: eq_tensor" fixture's values exactly. *)
