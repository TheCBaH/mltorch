(* The Native4D Symbolic half: the Native4D twin of
   ...
   exists, and none is needed. Native4D's own Symbolic safety is proven here
   rather than reasoned from the shared [Kernel.materializable] gate, for this
   subgraph: [inv] (the [Bitwise_not] output) is shared by
