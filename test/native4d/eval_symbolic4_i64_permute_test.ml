(* The Native4D twin of `test/native/eval_symbolic_i64_permute_test.ml` -- see
...
let%expect_test
    "Eval_symbolic4: I64 Arange4 -> Permute4 builds two chained Stage_i64s" =
...
(* Same [~outputs:[]] sidestep [eval_symbolic4_i64_reshape_test.ml]'s own
   second test already documents -- see that file's comment. *)
let%expect_test
    "Symbolic4 -> Kernel: I64 Arange4 -> Permute4 survives \
