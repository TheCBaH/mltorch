...
(* Native4D twin of [factory_test.ml]'s "arange with exact params stays
   exact past 2^53": [Eval_direct4]'s own D02 fix ([value_i64_exact]) and
...
let%expect_test "direct4 = symbolic4, bitwise, per op" =
  ...
  [%expect
    ...
    eq_scalar              direct = symbolic
    eq_tensor              direct = symbolic
    gt_scalar              direct = symbolic
    ne_scalar              direct = symbolic
    ne_tensor              direct = symbolic
    ...
    lstm                   out0 direct = symbolic
    lstm                   out1 direct = symbolic
    lstm                   out2 direct = symbolic
