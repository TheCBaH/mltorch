...
(* Rank<2 on either side, or unequal ranks, is still explicitly out of scope
   ...
   shapes, matching [Op_bridge]'s own diagnostic. *)
let%expect_test "matmul.default rejects unequal operand ranks" =
  dump "rank mismatch:" (prog ~x_sizes:[ 2; 2; 3 ] ~other_sizes:[ 3; 2 ] ());
  [%expect
    {|
    rank mismatch:
      malformed PT2 graph: matmul.default: both operands must be rank>=2 and of equal rank, got self=[2, 2, 3] other=[3, 2] |}]
