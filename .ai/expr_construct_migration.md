...
This is the `Expr`/Region-level counterpart of [adding a Native op](native_add_op.md) (which covers adding a
...
suffices. That assumption failing silently is the hazard: the splice appears to work, and
the collision only shows up as a captured binder in a context far from the splice site.
