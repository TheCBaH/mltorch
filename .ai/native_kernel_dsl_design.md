...
**`Value.t.computation`'s type since project step 19**: the sketch above
predates shared multi-output computation; `computation` is now
`Region_group.Ref.t = Grouped of Region_group.t * int | Solo of
Region_program.t`, not a bare `Region_program.t` (`Stage_program.Stage.t`
carries the same type, for the same reason). `Kernel.create`'s per-value
`check`/`preflight` and `max_scan_updates_total` aggregation project a
`Grouped` value to an ordinary `Region_program.t`
(`Region_group.Ref.project`) before applying the unchanged checks; a `Solo`
value is unaffected. `Kernel_eval.execute`'s eager materialize loop
(`run`/`run_plan`) partitions `Kernel.Value.t` into maximal runs sharing one
physical `Region_group.t` (`Region_group.runs`) and evaluates each run's
shared locals once, reading every stored member off that one evaluation,
instead of projecting and materializing each member independently;
`value_at`'s on-demand scalar path still projects per member (design record
§5.3's disclosed exception). See `native_multi_output_design.md` §5.
