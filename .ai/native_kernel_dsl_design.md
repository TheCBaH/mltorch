...
Status: **Phase 1 implemented**, August 2026; Phases 2–5 proposed.
...
capture-safe one-edge elaboration (`kernel_elab.ml`), and the first fusion
planner (`fusion_plan.ml`). `Expr` gained three composition primitives —
`Fold.loads`, `Fold.intrinsic_sources` and `Rewrite.substitute_loads` — and
`Quant.t` became abstract. Conv2D followed by Add executes with the conv's
buffer eliminated, bit-for-bit identical to materializing both stages.

`Compute`, the scheduler and the transformation verifier are unchanged, and the
Kernel layer is additive: nothing existing routes through it yet.
