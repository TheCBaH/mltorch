...
op8-impl.md's commit 0 spiked this closure before any Native code landed, on
the user's explicit instruction (a link failure here means re-scope, not
proceed). Ten new sources, in two batches:

- **the target itself**: `transformers/attention.cpp` (the composite body —
  a straight-line switch over every `SDPBackend`, so flash, math, cuDNN,
  efficient and overrideable are all *referenced* even though only flash is
  reachable from the supported scope) and `transformers/sdp_utils_cpp.cpp`
  (the backend gate); `cpu/FlashAttentionKernel.cpp` (the oracle kernel, F4 of
  `op8-impl.md`) in the CAP list.
- **is_causal's and enable_gqa's straight-line dependencies**, unreachable
  from the supported scope (both are typed rejections at the importer
  boundary) but still compiled code the switch's math arm references:
  `TriangularOps.cpp` (`.tril()`), `Repeat.cpp` (`repeat_interleave_symint`)
  and `TensorAdvancedIndexing.cpp` (`masked_fill`, whose kernel is
  `cpu/IndexKernel.cpp` in the CAP list) — matching `pad`'s reflect/replicate
  precedent above: a straight-line reference must link even when the engine
  will never take that branch.
- **`gen/ATen/CompositeViewCopyKernels.cpp`, the one nobody would have
  guessed.** `RegisterCompositeExplicitAutograd_0.cpp` (already linked)
  shares ONE translation-unit-wide registration function with
  `_triton_scaled_dot_attention`'s autogenerated `.out` composite wrapper —
  an unrelated triton-attention op that happens to share a shard. That
  wrapper's actual body (not a hand-written `native/` function at all) lives
  in `CompositeViewCopyKernels.cpp`, a generated file torchgen emits
  project-wide. `dc790ee`'s `pad` surprise was the precedent for spiking
  first rather than trusting the source-list prediction; this is the same
  lesson from the opposite direction — the missing piece was in `gen/`, not
  in a `native/` source anyone would think to grep for.
- **two stubs removed as duplicates**: `nonzero_cpu` and `index_select_cpu_`
  were in `atg_stubs.cpp` only because nothing else pulled
  `TensorAdvancedIndexing.cpp` in; with it real, the stubs are duplicate
  symbols, exactly `constant_pad_nd`'s story above.

Measured, not assumed (`make build`'s `libaten_core.a` / `dllaten_core.so`,
240 → 248 objects): **+8.3 MB (+10%) in the archive, +4.6 MB (+13%) in the
shared library.** The archive figure overstates what most consumers pay: the
OCaml executable link is `-fuse-ld=lld -Wl,--gc-sections`
(`lib/aten/dune`), so an executable that never calls
`atg_scaled_dot_product_attention` links none of this closure at all — the
whole point of the section-split SRCS_GLUE/SRCS_CAP lists above.

The oracle identity (F4 of `op8-impl.md`) could not be read from a log: the
selector (`sdp::select_sdp_backend_cpp`) returns immediately on success with
no diagnostic, so the spike built a scratch `extern "C"` probe around it
(never committed — see `op8-impl.md` commit 0) rather than trusting rank
alone to pin the backend, which it does not: three further gates (`Ev = E`,
mask shape, mask dtype) matter and each is a silent fallback to the `math`
