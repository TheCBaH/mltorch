...
let run model input reference =
  ...
  let* archive =
    Pt2_archive.open_pt2 model |> Core.map_error (fun e -> `Archive e)
  in
  let* input =
    Pt2_archive.load_pt input |> Core.map_error (fun e -> `Archive e)
  in
  let* result =
    Interp.run archive input |> Core.map_error (fun e -> `Interp e)
  in
  match Tensor_bridge.of_aten result with
  | Error message -> Core.fail (`Tensor_bridge message)
  | Ok output -> (
      match Graph_json.encode_tensor ~format:Jsont.Indent output with
      | Error message -> Core.fail (`Encode message)
      | Ok json ->
...
let () =
  ...
  | [| _; model; input; output |] -> (
      ...
      | Ok () -> ()
      | Error e ->
          Format.eprintf "aten_graph_ref: %a@." (Core.Error.pp pp_error) e;
          exit 1)
  | _ ->
